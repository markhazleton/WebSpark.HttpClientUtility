# Feature Specification: Batch Execution Orchestration

**Feature Branch**: `004-batch-execution-orchestration`
**Created**: 2026-03-16
**Status**: Draft
...
- Q: Canonical term for a target API endpoint with a base URL? → A: Environment (not "instance").
- Q: Does `{{encoded_user_name}}` apply URL encoding before substitution? → A: No — the token name is legacy only; the raw user identifier is inserted as-is and URL encoding remains the consumer's responsibility.
- Q: How does the system decide whether a custom HTTP method may include a request body? → A: The request definition includes explicit metadata indicating whether the method is body-capable; custom methods only send a body when that metadata is true.
- Q: How should the demo page satisfy the constitution's testing requirements? → A: Add automated MSTest coverage for the controller and demo run-tracking service, then keep manual browser validation as supplemental acceptance coverage.
...
**MSTest Coverage**: Unit tests must verify placeholder replacement in URL paths, request bodies, and combined scenarios. Edge cases for missing keys, empty values, nested/escaped braces, and the legacy `{{encoded_user_name}}` token's raw non-encoded substitution must be covered.
...
**MSTest Coverage**: Tests must verify correct combinatorial expansion (environment count x user count x request count x iteration count), concurrency throttling via semaphore, and cancellation token propagation.
...
5. **Given** an optional progress callback is provided, **When** each request completes during batch execution, **Then** the callback receives an update containing the completed count, total planned count, and a running statistics snapshot.
...
### User Story 6 - Interactive Web Demo (Priority: P6)

As a developer evaluating the library, I want to see batch execution orchestration demonstrated in the WebSpark.HttpClientUtility.Web demo application, so I can understand how to configure environments, users, and request templates, execute a batch, and interpret the resulting statistics — all through an interactive web UI.

**Why this priority**: The demo page is not a library feature but a showcase. It depends on P1-P3 being implemented first since it exercises template substitution, batch execution, and statistics collection through a web form.

**Independent Test**: Can be tested by navigating to the BatchExecution page, filling in the pre-populated form with sample environments and requests, clicking execute, and verifying that the results panel displays statistics including counts and per-environment breakdowns.

**MSTest Coverage**: Automated MSTest coverage must verify controller request validation, capped run orchestration, progress polling responses, and demo run state transitions. Manual verification via the running web application remains required as supplemental end-to-end confirmation.

**Acceptance Scenarios**:

1. **Given** the WebSpark.HttpClientUtility.Web application is running, **When** a user navigates to the BatchExecution page, **Then** the page displays a form with fields for environments (base URLs), user contexts, request templates, iteration count, and concurrency limit.
2. **Given** the demo form is pre-populated with 2 sample environments and 2 sample GET request templates, **When** the user clicks execute without modification, **Then** a capped batch runs (maximum ~50 requests) and the results panel shows total/success/failure counts, P50/P95/P99 response times, and per-environment breakdown.
3. **Given** the batch execution is in progress, **When** results begin arriving, **Then** the statistics panel updates to reflect completed requests (not delayed until the entire batch finishes).
4. **Given** the demo form allows the user to add custom request templates with `{placeholder}` tokens and a properties dictionary, **When** these are submitted, **Then** the template substitution is applied and the executed URLs reflect the substituted values.
...
- How does the system handle template placeholders that contain special characters (braces within values, URL-unsafe characters)? Values should be substituted as-is; the legacy `{{encoded_user_name}}` token also inserts the raw user identifier without additional encoding, and URL encoding is the consumer's responsibility.
...
- **FR-002**: The template engine MUST support the legacy `{{encoded_user_name}}` special token pattern, replacing it with the raw user-context-provided identifier without applying additional URL encoding.
...
- **FR-004**: The library MUST provide a batch execution orchestrator that accepts collections of environments (base URLs), user contexts (properties + identifier), and request definitions (method, path template, body template, and explicit metadata describing whether the method may include a request body).
...
- **FR-008**: The library MUST support all standard HTTP methods (GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS) and custom methods via arbitrary method name strings.
- **FR-009**: For methods that support a request body (POST, PUT, PATCH, and custom methods explicitly marked as body-capable in the request definition metadata), the system MUST attach the template-substituted body content. For methods that do not require a body (GET, DELETE, HEAD, OPTIONS, and custom methods not marked as body-capable), no body MUST be attached.
...
- **Execution Configuration**: Represents the full batch run setup — collections of environments, user contexts, request definitions, iteration count, and concurrency limit. Acts as the top-level input to the batch orchestrator.
- **Environment**: A target API endpoint defined by a base URL and a display name. Multiple environments enable cross-environment comparison.
...
- **Request Definition**: A template for an HTTP request — HTTP method, path template, body template, and explicit metadata about whether the method may include a body for custom-method scenarios.
- **Execution Result**: The outcome of a single request — environment identifier, request path, HTTP method, user identifier, success/failure, status code, response hash, duration, timestamp, and related batch metadata. Delivered incrementally to the output sink.
- **Execution Statistics**: Aggregated metrics from a complete batch run — counts, percentiles, and multi-dimensional breakdowns. Returned upon batch completion.
- **Batch Progress**: A lightweight progress snapshot delivered incrementally during execution — completed count, total planned count, and a running statistics summary. Consumed via the standard progress reporting pattern.
